...
function l1ChainForId(chainId: number): typeof mainnet | typeof sepolia | typeof foundry {
...
function parseAmountWei(chainId: number): bigint {
    ...
    return MAINNET_DEFAULT_AMOUNT;
}

function getFeeJuiceTokenAddress(chainId: number): `0x${string}` {
    if (!isMainnetLike(chainId)) {
        throw new Error(`This script is mainnet-only. Expected L1 chain id 1 but got ${chainId}.`);
    }
...
}): { leafIndex: bigint; messageHash: Fr } {
    ...
    for (const eventName of eventNames) {
        try {
            ...
            if (typeof args.index === 'bigint') {
                const key = (decoded.args as unknown as { key?: `0x${string}` }).key;
                if (!key) {
                    throw new Error('Deposit event missing key field.');
                }
                return { leafIndex: args.index, messageHash: Fr.fromHexString(key) };
            }
        } catch (err) {
            lastError = err;
        }
...
async function ensureErc20Allowance(opts: {
    l1Client: ReturnType<typeof createExtendedL1Client>;
    token: `0x${string}`;
    owner: `0x${string}`;
    spender: `0x${string}`;
    ...
    opts.logger.info(`Approving FeeJuicePortal to spend FeeJuice (amount=${opts.amount.toString()} wei)...`);
...
async function main() {
    ...
    if (!isMainnetLike(chainId)) {
        throw new Error(`This script is mainnet-only. Current L1 chain id from config is ${chainId}.`);
    }
    const l1RpcUrls = Array.from(
        new Set([...splitRpcUrls(optionalEnv('L1_RPC_URL')), getL1RpcUrl()]),
    );
    const l1PrivateKey = normalizePrivateKey(requiredEnv('L1_PRIVATE_KEY'));
    ...
    if (!first) {
        throw new Error(
            'No account found in this wallet/PXE. Create and deploy a Schnorr account first (e.g. via aztec-wallet), then rerun.',
        );
    }
    const claimerAddress = first.item;

    logger.info(`Using L1 chain id: ${chainId}`);
    logger.info(`Using L1 RPCs: ${l1RpcUrls.join(', ')}`);
    ...
    const l1Client = createExtendedL1Client(l1RpcUrls, l1PrivateKey, l1ChainForId(chainId));
    ...
    const { leafIndex, messageHash } = extractDepositLeafIndexFromReceipt({
        logs: receipt.logs,
