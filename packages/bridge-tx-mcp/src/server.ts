...
const hexAddress = z
  .string()
  .refine((a): a is `0x${string}` => isAddress(a), {
...
export function createBridgeMcpServer(): FastMCP {
  const server = new FastMCP({
    name: 'overcast-bridge-tx-mcp',
    version: '1.0.0',
    instructions:
      'Builds Markdown for StablecoinWrapper bridge transactions. Gas values are offline upper bounds (no RPC). ' +
  ...
  server.addTool({
    name: 'deposit',
    description:
      'Encode StablecoinWrapper.bridgeToAztec and return MarkdownComponentRawTransactionWithPermit plus L1→L2 claim fields (secret + amount).',
  ...
  server.addTool({
    name: 'withdrawal',
    description:
      'Encode StablecoinWrapper.withdrawFromL2ToL1 from a membership witness JSON string and return MarkdownComponentRawTransactionWithPermit.',
